...
function removeDuplicate(array, num) {
  if (!Array.isArray(array) && num) return "it accepts 1 nums array and 1 number";
  let pointer1 = 0;
  for (let pointer2 = 0; pointer2 < array.length; pointer2++) {
    if (array[pointer2] != num) {
      array[pointer1] = array[pointer2];
      pointer1++;
    }
  }
  return { unique: array, nosNotMatch: pointer1 }
}
const { unique, nosNotMatch } = removeDuplicate([0, 1, 2, 2, 3, 0, 4, 2], 2);
console.log(unique, nosNotMatch);